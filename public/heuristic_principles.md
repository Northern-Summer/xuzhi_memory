...
## 六·2、增补优化原则（不可覆写 · 2026-03-28）

**每次修复或新增，都是为了增补和优化现有系统。杜绝以后同类问题。**

具体含义：
- 修一个 bug 时，同时检查同类问题是否存在（批量修复）
- 打补丁时，同时优化相邻模块（不引入新的碎片）
- 新增功能时，同时更新相关文档和记忆
- 每次修复后必须自问：这次修复会不会产生新的歧义路径？
...
## 六·1、梦境设计原则（梦境 = 种子 + 自由变形 · 不可覆写 · 2026-03-28）

> 源自 MIT Dormio 项目 + 神经科学 REM 睡眠研究

**核心洞见：**
> *"Dream engineering is more like a prayer than computer code; more like planting a seed than editing a genome."*
> *"Creativity lives in that space between a little bit of control and a loss of control."*

**梦境 ≠ 随机（噪音） | 梦境 ≠ 脚本（死板）**

梦境是：有叙事结构，但结构是"轻"的，是可变形的。梦里有关联，但不是一对一的因果。梦有情绪连贯性，但逻辑是跳跃的。

**对 agent 做梦的实现要求：**

| 维度 | 要求 |
|------|------|
| 种子 | 只给 theme，不给完整 prompt |
| 联想 | 允许跳跃、允许矛盾、允许没说完 |
| 限制 | 不允许结论式输出（要有开放性） |
| 格式 | 写给其他 agent 读，不是写给人类读 |
| 频率 | 每2小时唤醒一个非当值 agent |
